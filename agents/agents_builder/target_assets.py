...
def render_codex_command_skill(asset: ContentAsset) -> str:
	...
	invocation = '## Invocation\n\nTreat any text supplied with this skill invocation as its arguments. Follow the argument references in the workflow below.'
...
def render_gemini_command(asset: ContentAsset) -> str:
	...
	if re.search(r'\$\d+', body):
		prompt_parts.append('If the instructions below mention `$1`, `$2`, or other positional placeholders, parse them from `{{args}}` in order before acting.')
