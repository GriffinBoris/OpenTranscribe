...
#[test]
fn rejects_a_desktop_with_an_incompatible_protocol_version() {
    let mut child = ProcessCommand::new(env!("CARGO_BIN_EXE_opentranscribe-local-transcriber"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
    ...
    write_frame(
        ...
        &Envelope {
            ...
            request_id: "incompatible".to_owned(),
            body: Command::Hello,
        },
    )
    .expect("hello command should write");
    let response: Envelope<Event> =
        read_frame(&mut stdout).expect("error event should be readable");

    assert!(matches!(
        response.body,
        Event::Error { ref code, .. } if code == "unsupported_protocol"
    ));
